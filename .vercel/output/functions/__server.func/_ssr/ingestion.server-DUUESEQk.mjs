...
import { t as CATEGORIES } from "./categories-CY0cJTXM.mjs";
import { r as lookupWords } from "./dictionary.server-CJ6qJACk.mjs";
import { getCategoryFallbackImage, orJson, pexelsImage, pexelsVideo } from "./openrouter.server-DBp1OwsK.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/ingestion.server-DUUESEQk.js
...
async function runIngestion(opts) {
	...
	for (const item of processed) {
		...
		try {
			const videoSubject = (p.title || raw.title).replace(/[^A-Za-z0-9\s]/g, " ").split(/\s+/).filter((w) => w.length >= 4 && !/^(the|and|for|from|with|after|about|into|amid|says|will|been|have|this|that|their|them)$/i.test(w)).slice(0, 5).join(" ");
			if (videoSubject) {
...
async function backfillVideos(opts) {
	const supabase = adminClient();
	const limit = Math.min(Math.max(opts?.limit ?? 20, 1), 50);
	const { data: rows } = await supabase.from("articles").select("id, title, category, cover_image_url, cover_video_url").is("cover_video_url", null).eq("is_published", true).order("published_at", { ascending: false }).limit(limit);
	const items = rows ?? [];
	let updated = 0;
	let failed = 0;
	await pMap(items, 3, async (row) => {
		try {
			const vid = await pexelsVideo(row.title.replace(/[^A-Za-z0-9\s]/g, " ").split(/\s+/).filter((w) => w.length >= 4 && !/^(the|and|for|from|with|after|about|into|amid|says|will|been|have|this|that|their|them|over|more|than|what|when|where|which|while|also|could|would|should|might|must|does|doing|done|made|make|gets|getting|goes|going|come|coming|takes|taking|finds|found|find|reveals|revealed|shows|shown|show|study|report|according|image|photo|getty|reuters|caption|via|advertisement|story|article|read|more|click|subscribe|sign)\b/i.test(w)).slice(0, 4).join(" ") || row.category || "news");
			if (vid?.videoUrl) {
				const { error } = await supabase.from("articles").update({ cover_video_url: vid.videoUrl }).eq("id", row.id);
				if (error) failed++;
				else updated++;
			} else {
				const fallbackVid = await pexelsVideo(row.category || "news");
				if (fallbackVid?.videoUrl) {
					const { error } = await supabase.from("articles").update({ cover_video_url: fallbackVid.videoUrl }).eq("id", row.id);
					if (error) failed++;
					else updated++;
				} else failed++;
			}
			...
			failed++;
			console.error("[backfill-videos] error:", e.message);
		}
	});
	const { count } = await supabase.from("articles").select("id", {
		count: "exact",
		head: true
	}).is("cover_video_url", null).eq("is_published", true);
	return {
		attempted: items.length,
		updated,
		failed,
...
//#endregion
export { backfillQuizzes, backfillVideos, backfillVocabulary, reprocessBatch, runIngestion };
