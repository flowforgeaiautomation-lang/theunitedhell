...
import { t as translateVisibleText } from "./translation.functions-ChC1Wo0w.mjs";
import { a as categoryLabel } from "./categories-CY0cJTXM.mjs";
import { t as supabase } from "./client-d8MeWTAO.mjs";
import { a as getLikedComments, c as listComments, l as postReflection, o as getRelated, t as bumpLike } from "./articles.functions-Cbu5EOsE.mjs";
import { i as useReadingPrefs, r as articleQ, t as Route } from "./article._slug-DJfXnBh7.mjs";
import { a as toggleBookmark, o as toggleLike, t as getMyInteractions } from "./interactions.functions-CyjaS5xo.mjs";
import { n as fallbackCoverUrl, t as SmartImage } from "./SmartImage-C6Yr1ccR.mjs";
import { A as RotateCcw, B as Maximize, D as Share2, Et as BookOpen, K as Info, L as Minimize, M as Play, N as Pause, Q as Globe, R as MessageCircle, S as Sparkles, St as Building2, T as SkipBack, Tt as BookmarkCheck, U as Lightbulb, X as Hash, Y as Heart, _t as ChevronLeft, b as Square, et as Gauge, gt as ChevronRight, ht as ChevronUp, jt as ArrowBigUp, k as Search, l as Users, m as TrendingUp, mt as CircleCheck, o as VolumeX, p as Trophy, pt as CircleX, r as X, s as Volume2, st as Earth, vt as ChevronDown, w as SkipForward, wt as Bookmark } from "../_libs/lucide-react.mjs";
import { t as motion } from "../_libs/framer-motion.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/article._slug-BU1VqF_M.js
...
var CURATED_VOCAB = [
	...
	},
	{
		...
		simpleExplanation: "Talking to make a deal.",
		example: "The negotiation lasted three days.",
		synonyms: [
			"discussion",
			"bargaining",
			"dialogue"
	...
	},
	{
		...
		simpleExplanation: "When most people agree.",
...
function generateLocalVocabFallback(text) {
	if (!text || text.trim().length < 10) return CURATED_VOCAB.slice(0, 6);
	const words = text.replace(/[^a-zA-Z\s]/g, " ").split(/\s+/).map((w) => w.trim().toLowerCase()).filter((w) => w.length >= 7 && w.length <= 14 && !STOPWORDS.has(w));
	const freq = /* @__PURE__ */ new Map();
	for (const w of words) freq.set(w, (freq.get(w) || 0) + 1);
	const sorted = [...freq.entries()].sort((a, b) => b[1] - a[1]).slice(0, 20);
	const known = new Map(CURATED_VOCAB.map((v) => [v.word, v]));
	const picks = [];
	const used = /* @__PURE__ */ new Set();
	for (const [w] of sorted) {
		const curated = known.get(w);
		if (curated && !used.has(w)) {
			picks.push(curated);
			used.add(w);
		}
	}
	for (const v of CURATED_VOCAB) {
