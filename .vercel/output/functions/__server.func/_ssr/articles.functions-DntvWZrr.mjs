...
//#region node_modules/.nitro/vite/services/ssr/assets/articles.functions-DntvWZrr.js
...
var listArticles = createServerFn({ method: "GET" }).inputValidator((d) => objectType({
	...
	if (rpcError) {
		...
		return {
			items: (fbData ?? []).map((r) => decodeSummary(r)),
			nextCursor: void 0,
			hasMore: false
		};
	}
...
var getFeatured = createServerFn({ method: "GET" }).handler(getFeatured_createServerFn_handler, async () => {
	const { data, error } = await publicClient().rpc("list_articles_sorted", {
		p_sort: "recent",
		p_limit: 100
	});
...
var getArticleBySlug = createServerFn({ method: "GET" }).inputValidator((d) => objectType({ slug: stringType().min(1) }).parse(d)).handler(getArticleBySlug_createServerFn_handler, async ({ data }) => {
	...
	const summary = (rpcData?.items ?? []).find((a) => a.slug === data.slug);
	...
	const article = {
		...row,
		cover_video_url: summary.cover_video_url
	};
	try {
...
var getRelated = createServerFn({ method: "GET" }).inputValidator((d) => objectType({
	...
	if (error) throw new Error(error.message);
	let result = dedupeSummaries((sameCat ?? []).filter((r) => r.slug !== data.excludeSlug), data.limit);
	if (result.length < data.limit) {
		const { data: recent } = await supabase.rpc("list_articles_sorted", {
			p_sort: "recent",
			p_limit: data.limit * 3
		});
		const recentRows = (recent?.items ?? []).filter((r) => r.slug !== data.excludeSlug);
...
var searchArticles = createServerFn({ method: "GET" }).inputValidator((d) => objectType({ q: stringType().min(1).max(120) }).parse(d)).handler(searchArticles_createServerFn_handler, async ({ data }) => {
	const { data: rpcData, error } = await publicClient().rpc("list_articles_sorted", {
		p_sort: "recent",
		p_limit: 500
	});
	if (error) throw new Error(error.message);
	...
	return (result?.items ?? []).filter((a) => (a.title?.toLowerCase().includes(term) ?? false) || (a.dek?.toLowerCase().includes(term) ?? false) || (a.category?.toLowerCase().includes(term) ?? false)).slice(0, 40).map((r) => decodeSummary(r));
});
...
var getBriefingToday = createServerFn({ method: "GET" }).handler(getBriefingToday_createServerFn_handler, async () => {
	...
	const buildFromArticles = async () => {
		...
		const { data: latestRpc } = await supabase.rpc("list_articles_sorted", {
			p_sort: "recent",
			p_limit: 80
		});
		const latestRows = dedupeSummaries(latestRpc?.items ?? [], 60);
