...
//#region node_modules/.nitro/vite/services/ssr/assets/articles.functions-D1bxZ4ng.js
...
var DEFAULT_VIDEO = "https://videos.pexels.com/video-files/3571264/3571264-uhd_2560_1440_30fps.mp4";
function videoForCategory(category) {
...
var listArticles = createServerFn({ method: "GET" }).inputValidator((d) => objectType({
	...
	if (data.cursor) {
		...
		if (sep > 0) {
			data.cursor.slice(0, sep);
			data.cursor.slice(sep + 1);
			if (data.sort === "recent") {}
		}
	}
	let query = supabase.from("articles").select("id,slug,title,dek,category,subcategory,cover_image_url,read_time_minutes,country_code,featured_slot,published_at,created_at,view_count,like_count,bookmark_count,comment_count").eq("is_published", true);
	if (data.todayOnly) {
	...
	if (data.sort === "trending") query = query.order("view_count", { ascending: false }).order("id", { ascending: false });
	else if (data.sort === "most_read") query = query.order("view_count", { ascending: false }).order("id", { ascending: false });
	else if (data.sort === "most_saved") query = query.order("bookmark_count", { ascending: false }).order("id", { ascending: false });
	...
	return {
		items: (rows ?? []).map((r) => decodeSummary(r)),
		nextCursor: void 0,
		hasMore: false
	};
});
...
var getFeatured = createServerFn({ method: "GET" }).handler(getFeatured_createServerFn_handler, async () => {
	const { data, error } = await publicClient().from("articles").select("id,slug,title,dek,category,subcategory,cover_image_url,read_time_minutes,country_code,featured_slot,published_at,created_at,view_count,like_count,bookmark_count,comment_count").eq("is_published", true).not("featured_slot", "is", null).order("published_at", { ascending: false }).limit(20);
	if (error) throw new Error(error.message);
...
var getArticleBySlug = createServerFn({ method: "GET" }).inputValidator((d) => objectType({ slug: stringType().min(1) }).parse(d)).handler(getArticleBySlug_createServerFn_handler, async ({ data }) => {
	const supabase = publicClient();
	const { data: row, error } = await supabase.from("articles").select("id,slug,title,dek,category,subcategory,cover_image_url,cover_image_prompt,read_time_minutes,trust_score,source_count,sources,story,body,country_code,featured_slot,is_published,published_at,view_count,like_count,bookmark_count,comment_count,created_by,created_at,updated_at").eq("slug", data.slug).eq("is_published", true).maybeSingle();
	if (error) throw new Error(error.message);
	if (!row) return null;
	const article = decodeSummary(row);
	try {
...
var getRelated = createServerFn({ method: "GET" }).inputValidator((d) => objectType({
	...
	const supabase = publicClient();
	const { data: sameCat, error } = await supabase.from("articles").select("id,slug,title,dek,category,subcategory,cover_image_url,read_time_minutes,country_code,featured_slot,published_at,created_at,view_count,like_count,bookmark_count,comment_count").eq("is_published", true).eq("category", data.category).neq("slug", data.excludeSlug).order("published_at", { ascending: false }).limit(data.limit);
	if (error) throw new Error(error.message);
	let result = dedupeSummaries((sameCat ?? []).map((r) => decodeSummary(r)), data.limit);
	if (result.length < data.limit) {
		const { data: recent } = await supabase.from("articles").select("id,slug,title,dek,category,subcategory,cover_image_url,read_time_minutes,country_code,featured_slot,published_at,created_at,view_count,like_count,bookmark_count,comment_count").eq("is_published", true).neq("slug", data.excludeSlug).order("published_at", { ascending: false }).limit(data.limit * 3);
		result = dedupeSummaries([...result, ...(recent ?? []).map((r) => decodeSummary(r))], data.limit);
	}
	return result;
});
...
var searchArticles = createServerFn({ method: "GET" }).inputValidator((d) => objectType({ q: stringType().min(1).max(120) }).parse(d)).handler(searchArticles_createServerFn_handler, async ({ data }) => {
	const { data: rows, error } = await publicClient().from("articles").select("id,slug,title,dek,category,subcategory,cover_image_url,read_time_minutes,country_code,featured_slot,published_at,created_at,view_count,like_count,bookmark_count,comment_count").eq("is_published", true).or(`title.ilike.%${data.q}%,dek.ilike.%${data.q}%,category.ilike.%${data.q}%`).order("published_at", { ascending: false }).limit(40);
	if (error) throw new Error(error.message);
...
var getBriefingToday = createServerFn({ method: "GET" }).handler(getBriefingToday_createServerFn_handler, async () => {
	...
	const buildFromArticles = async () => {
		...
		const fetchCat = async (cats, limit) => {
			const { data: rows } = await supabase.from("articles").select(safeCols).eq("is_published", true).in("category", cats).order("published_at", { ascending: false }).limit(limit);
			return dedupeSummaries((rows ?? []).map((r) => decodeSummary(r)), limit);
		};
		const { data: latestRows } = await supabase.from("articles").select(safeCols).eq("is_published", true).order("published_at", { ascending: false }).limit(80);
		dedupeSummaries((latestRows ?? []).map((r) => decodeSummary(r)), 60);
...
var getArticleImages = createServerFn({ method: "GET" }).inputValidator((d) => objectType({
	...
	const { pexelsImage } = await import("./openrouter.server-DBp1OwsK.mjs");
