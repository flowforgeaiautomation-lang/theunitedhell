...
import { t as requireSupabaseAuth } from "./auth-middleware-lrRllEyr.mjs";
import { i as enumType, n as arrayType, o as objectType, s as stringType } from "../_libs/zod.mjs";
import { t as createServerRpc } from "./createServerRpc-B90ckaqP.mjs";
import { orJson } from "./openrouter.server-DBp1OwsK.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/quiz.functions-D_W68Vsn.js
...
var listSavedWords = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).handler(listSavedWords_createServerFn_handler, async ({ context }) => {
	const { supabase, userId } = context;
	const { data, error } = await supabase.from("saved_words").select("word,meaning,pronunciation,part_of_speech,example,synonyms,antonyms,difficulty,simple_explanation,context_in_article,word_origin,created_at").eq("user_id", userId).order("created_at", { ascending: false });
	if (error) throw new Error(error.message);
	return data ?? [];
});
