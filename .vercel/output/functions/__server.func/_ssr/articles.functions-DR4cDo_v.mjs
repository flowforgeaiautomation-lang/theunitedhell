...
//#region node_modules/.nitro/vite/services/ssr/assets/articles.functions-DR4cDo_v.js
...
var summaryCols = "id,slug,title,dek,category,subcategory,cover_image_url,read_time_minutes,country_code,featured_slot,published_at,created_at,view_count,like_count,bookmark_count,comment_count,trending_score";
var NAMED_ENTITIES = {
	amp: "&",
	lt: "<",
	gt: ">",
...
function cleanTitle(text) {
	...
	if (!cleaned) return void 0;
	if ((cleaned.match(/'/g) || []).length % 2 === 1) if (cleaned.startsWith("'") && !cleaned.endsWith("'")) cleaned += "'";
...
async function normalizeArticle(article) {
	...
	let finalVocab = (currentStory.vocabulary?.map((v) => ({
		...
		contextInArticle: dec(v.contextInArticle) || dec(v.context_in_article) || void 0,
		...
		wordOrigin: dec(v.wordOrigin) || dec(v.word_origin) || void 0
	})).filter((v) => v.word && v.meaning) || []).slice(0, 10);
	...
	if (finalVocab.length < 5) {
		...
		if (finalVocab.length < 5) {
			const moreFallback = await generateFallbackVocab([
				article.title,
				article.dek,
				summary,
				mainStory,
				...
				decClean(currentStory.expert_analysis),
				decClean(currentStory.why_it_matters),
				decClean(currentStory.historical_context),
				decClean(currentStory.future_outlook),
				...currentStory.key_developments || [],
				...currentStory.quick_insights || [],
				...currentStory.reader_takeaways || []
			].filter(Boolean).join(" "), finalVocab);
			if (moreFallback.length) {
				const existing = new Set(finalVocab.map((v) => v.word?.toLowerCase()));
				finalVocab = [...finalVocab, ...moreFallback.filter((v) => !existing.has(v.word?.toLowerCase()))].slice(0, 10);
				vocabWasRegenerated = true;
			}
		}
	}
...
var listArticles = createServerFn({ method: "GET" }).inputValidator((d) => objectType({
	...
	let cursorDate = null;
	let cursorId = null;
	let cursorSortVal = null;
	if (data.cursor) {
		const sep = data.cursor.lastIndexOf("|");
		if (sep > 0) {
			const sortValPart = data.cursor.slice(0, sep);
			cursorId = data.cursor.slice(sep + 1);
			if (data.sort === "recent") cursorDate = sortValPart;
			else cursorSortVal = sortValPart;
		}
	}
	let query = supabase.from("articles").select(summaryCols).eq("is_published", true);
	if (data.category) {
		const related = relatedCategorySlugs(data.category);
		if (related.length > 0) query = query.in("category", related);
		else query = query.eq("category", data.category);
	}
	...
	if (data.sort === "recent") if (cursorId && cursorDate) query = query.or(`published_at.lt.${cursorDate},and(published_at.eq.${cursorDate},id.lt.${cursorId})`).order("published_at", { ascending: false }).order("id", { ascending: false });
	else query = query.order("published_at", { ascending: false }).order("id", { ascending: false });
	else if (data.sort === "trending") if (cursorId && cursorSortVal !== null) query = query.or(`trending_score.lt.${cursorSortVal},and(trending_score.eq.${cursorSortVal},id.lt.${cursorId})`).order("trending_score", { ascending: false }).order("id", { ascending: false });
	else query = query.order("trending_score", { ascending: false }).order("id", { ascending: false });
	else if (data.sort === "most_read") if (cursorId && cursorSortVal !== null) query = query.or(`view_count.lt.${cursorSortVal},and(view_count.eq.${cursorSortVal},id.lt.${cursorId})`).order("view_count", { ascending: false }).order("id", { ascending: false });
	else query = query.order("view_count", { ascending: false }).order("id", { ascending: false });
	else if (data.sort === "most_saved") if (cursorId && cursorSortVal !== null) query = query.or(`bookmark_count.lt.${cursorSortVal},and(bookmark_count.eq.${cursorSortVal},id.lt.${cursorId})`).order("bookmark_count", { ascending: false }).order("id", { ascending: false });
	else query = query.order("bookmark_count", { ascending: false }).order("id", { ascending: false });
	query = query.limit(fetchLimit);
	const { data: rows, error } = await query;
	if (error) throw new Error(error.message);
	const allRows = (rows ?? []).map((r) => decodeSummary(r));
	...
	if (hasMore && deduped.length > 0) {
		...
		if (data.sort === "recent") nextCursor = `${last.published_at}|${last.id}`;
		else if (data.sort === "trending") nextCursor = `${last.trending_score ?? 0}|${last.id}`;
		else if (data.sort === "most_read") nextCursor = `${last.view_count}|${last.id}`;
		else if (data.sort === "most_saved") nextCursor = `${last.bookmark_count}|${last.id}`;
	}
	return {
		items: deduped,
		nextCursor,
...
var getFeatured = createServerFn({ method: "GET" }).handler(getFeatured_createServerFn_handler, async () => {
	const { data, error } = await publicClient().from("articles").select(summaryCols).eq("is_published", true).not("featured_slot", "is", null).order("published_at", { ascending: false }).order("id", { ascending: false });
	if (error) throw new Error(error.message);
...
var getRelated = createServerFn({ method: "GET" }).inputValidator((d) => objectType({
	...
	const { data: sameCat } = await supabase.from("articles").select(summaryCols).eq("is_published", true).eq("category", data.category).neq("slug", excludeSlug).order("published_at", { ascending: false }).order("id", { ascending: false }).limit(limit);
	let result = dedupeSummaries(sameCat ?? [], limit);
	if (result.length < limit) {
		const { data: currentRow } = await supabase.from("articles").select("country_code").eq("slug", excludeSlug).maybeSingle();
		const cc = currentRow?.country_code;
		if (cc) {
			const { data: byCountry } = await supabase.from("articles").select(summaryCols).eq("is_published", true).eq("country_code", cc).neq("slug", excludeSlug).order("published_at", { ascending: false }).order("id", { ascending: false }).limit(limit * 2);
			result = dedupeSummaries([...result, ...byCountry ?? []], limit);
		}
	}
	if (result.length < limit) {
		const { data: recent } = await supabase.from("articles").select(summaryCols).eq("is_published", true).neq("slug", excludeSlug).order("published_at", { ascending: false }).order("id", { ascending: false }).limit(limit * 3);
		result = dedupeSummaries([...result, ...recent ?? []], limit);
	}
	return result;
});
...
var searchArticles = createServerFn({ method: "GET" }).inputValidator((d) => objectType({ q: stringType().min(1).max(120) }).parse(d)).handler(searchArticles_createServerFn_handler, async ({ data }) => {
	...
	const { data: rows, error } = await supabase.from("articles").select(summaryCols).eq("is_published", true).or(`title.ilike.${term},dek.ilike.${term},category.ilike.${term}`).order("published_at", { ascending: false }).order("id", { ascending: false }).limit(40);
	if (error) throw new Error(error.message);
	return rows ?? [];
});
...
var getBriefingToday = createServerFn({ method: "GET" }).handler(getBriefingToday_createServerFn_handler, async () => {
	...
	const buildFromArticles = async () => {
		...
		const fetchCat = async (cats, limit) => {
			const { data: rows } = await supabase.from("articles").select(summaryCols).eq("is_published", true).in("category", cats).order("published_at", { ascending: false }).order("id", { ascending: false }).limit(limit);
			return dedupeSummaries(rows ?? [], limit);
		};
		const { data: latest } = await supabase.from("articles").select(summaryCols).eq("is_published", true).order("published_at", { ascending: false }).order("id", { ascending: false }).limit(80);
...
}).parse(d)).handler(bumpLike_createServerFn_handler, async ({ data }) => {
	const { data: result, error } = await publicClient().rpc("toggle_comment_like", {
...
var editComment_createServerFn_handler = createServerRpc({
	id: "cf1cdac7157a80d3d8e660904338d2a8a48851b7be185d96121c78cb1ce93e17",
	name: "editComment",
	filename: "src/lib/articles.functions.ts"
...
var editComment = createServerFn({ method: "POST" }).inputValidator((d) => objectType({
	commentId: stringType().uuid(),
	body: stringType().trim().min(1).max(4e3)
}).parse(d)).handler(editComment_createServerFn_handler, async ({ data }) => {
	const { error } = await publicClient().rpc("edit_comment_by_id", {
		p_comment_id: data.commentId,
		p_body: data.body
	});
	if (error) throw new Error(error.message);
...
var getLikedComments_createServerFn_handler = createServerRpc({
	id: "9fd012611d76e06ad7178d08790237fc82580098437e9480ae3b17fe3503def9",
	name: "getLikedComments",
	filename: "src/lib/articles.functions.ts"
}, (opts) => getLikedComments.__executeServer(opts));
var getLikedComments = createServerFn({ method: "GET" }).inputValidator((d) => objectType({
	articleId: stringType().uuid(),
	userId: stringType().uuid().nullable().optional()
}).parse(d)).handler(getLikedComments_createServerFn_handler, async ({ data }) => {
	const { data: result, error } = await publicClient().rpc("get_comment_likes_for_user", {
		p_article_id: data.articleId,
		p_user_id: data.userId ?? null
	});
	if (error) throw new Error(error.message);
...
var getArticleImages_createServerFn_handler = createServerRpc({
	id: "7e967bd6e25512f886a6238ee62a59673646afe9217936f4e5edf813bb90f109",
	name: "getArticleImages",
	filename: "src/lib/articles.functions.ts"
}, (opts) => getArticleImages.__executeServer(opts));
var getArticleImages = createServerFn({ method: "GET" }).inputValidator((d) => objectType({
	slug: stringType().min(1),
	title: stringType(),
	category: stringType()
}).parse(d)).handler(getArticleImages_createServerFn_handler, async ({ data }) => {
	const { pexelsImage } = await import("./openrouter.server-qFv0nSp-.mjs");
...
var listComments = createServerFn({ method: "GET" }).inputValidator((d) => objectType({
	...
	const { data: raw, error } = await publicClient().rpc("list_comments_by_article", {
		p_article_id: data.articleId,
...
//#endregion
export { bumpLike_createServerFn_handler, deleteCommentAnon_createServerFn_handler, editComment_createServerFn_handler, getArticleBySlug_createServerFn_handler, getArticleImages_createServerFn_handler, getBriefingToday_createServerFn_handler, getCountryStats_createServerFn_handler, getFeatured_createServerFn_handler, getLikedComments_createServerFn_handler, getRelated_createServerFn_handler, listArticles_createServerFn_handler, listComments_createServerFn_handler, postReflection_createServerFn_handler, searchArticles_createServerFn_handler };
