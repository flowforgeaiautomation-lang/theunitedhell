...
//#region node_modules/.nitro/vite/services/ssr/assets/articles.functions-XtISuwXh.js
...
var listArticles = createServerFn({ method: "GET" }).inputValidator((d) => objectType({
	...
	const items = (rows ?? []).map((r) => decodeSummary(r));
	...
	if (hasMore && items.length > 0) {
		...
		else nextCursor = `${last.view_count ?? 0}|${last.id}`;
	}
	return {
		items,
		nextCursor,
