...
import { r as lookupWords } from "./dictionary.server-BOE7fQZZ.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/articles.functions-Bvrnub0W.js
...
async function generateFallbackVocab(text, existing) {
	...
	return (await lookupWords([...freq.entries()].sort((a, b) => b[1] - a[1] || b[0].length - a[0].length).filter(([w]) => !existing.some((e) => e.word?.toLowerCase() === w)).slice(0, 12).map(([w]) => w))).filter((v) => v.meaning && !/an important (word|term) used in this story/i.test(v.meaning)).slice(0, 5);
}
