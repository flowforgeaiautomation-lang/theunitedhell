...
//#region node_modules/.nitro/vite/services/ssr/assets/translation.functions-DG_8yan8.js
...
async function aiTranslate(texts, target) {
	const { orChat } = await import("./openrouter.server-qFv0nSp-.mjs");
...
var translateVisibleText = createServerFn({ method: "POST" }).inputValidator((d) => objectType({
	...
	let translations = await googleTranslate(unique, data.target) ?? await deeplTranslate(unique, data.target) ?? await aiTranslate(unique, data.target);
	return Object.fromEntries(unique.map((text, i) => [text, translations[i] || text]));
});
//#endregion
export { translateVisibleText_createServerFn_handler };
