...
//#region node_modules/.nitro/vite/services/ssr/assets/articles.functions-B3XP0-4s.js
...
async function generateFallbackVocab(text, existing) {
	...
	return (await lookupWords([...freq.entries()].sort((a, b) => b[1] - a[1] || b[0].length - a[0].length).filter(([w]) => !existing.some((e) => e.word?.toLowerCase() === w)).slice(0, 20).map(([w]) => w))).filter((v) => v.meaning && v.meaning.length > 5 && !/an important (word|term) used in this story|a difficult word from the english language|^\s*of\s*$/i.test(v.meaning)).slice(0, 5);
}
async function normalizeArticle(article) {
	...
	const articleTextNorm = normalizeText([
		article.title,
		article.dek,
		summary,
		mainStory
	...
	const TRIVIAL_WORDS = /* @__PURE__ */ new Set([
		"possibly",
		"actually",
		"really",
		"very",
...
var getRelated = createServerFn({ method: "GET" }).inputValidator((d) => objectType({
	...
	const { data: rows, error } = await publicClient().from("articles").select(summaryCols).eq("is_published", true).eq("category", data.category).neq("slug", data.excludeSlug).order("published_at", { ascending: false }).order("id", { ascending: false }).limit(data.limit);
	if (error) throw new Error(error.message);
	return rows ?? [];
});
...
var DEFAULT_IMAGE_SET = CATEGORY_IMAGE_SETS.world;
