...
//#region node_modules/.nitro/vite/services/ssr/assets/articles.functions-sTDjwSS6.js
...
var bumpLike = createServerFn({ method: "POST" }).inputValidator((d) => objectType({ commentId: stringType().uuid() }).parse(d)).handler(bumpLike_createServerFn_handler, async ({ data }) => {
	const supabase = publicClient();
	const { data: row, error: fetchErr } = await supabase.from("comments").select("like_count").eq("id", data.commentId).maybeSingle();
	if (fetchErr) throw new Error(fetchErr.message);
	const newCount = (row?.like_count ?? 0) + 1;
	const { error } = await supabase.from("comments").update({ like_count: newCount }).eq("id", data.commentId);
	if (error) throw new Error(error.message);
	return {
		ok: true,
		likeCount: newCount
...
var deleteCommentAnon = createServerFn({ method: "POST" }).inputValidator((d) => objectType({ commentId: stringType().uuid() }).parse(d)).handler(deleteCommentAnon_createServerFn_handler, async ({ data }) => {
	const { error } = await publicClient().from("comments").delete().eq("id", data.commentId);
	if (error) throw new Error(error.message);
	return { deleted: true };
});
