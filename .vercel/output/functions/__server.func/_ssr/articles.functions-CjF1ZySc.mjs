...
//#region node_modules/.nitro/vite/services/ssr/assets/articles.functions-CjF1ZySc.js
...
var listArticles = createServerFn({ method: "GET" }).inputValidator((d) => objectType({
	...
	if (data.cursor) {
		...
		if (cursorDate && cursorId) q = q.or(`published_at.lt.${cursorDate},and(published_at.eq.${cursorDate},id.lt.${cursorId})`);
	}
	...
	const deduped = dedupeSummaries(hasMore ? allRows.slice(0, data.limit) : allRows, data.limit);
