...
//#region node_modules/.nitro/vite/services/ssr/assets/articles.functions-AHwvirA0.js
...
function generateFallbackVocab(text, title, existing) {
	...
	const words = text.replace(/[^a-zA-Z\s]/g, " ").split(/\s+/).filter((w) => w.length >= 5 && !STOP.has(w.toLowerCase()));
	const freq = /* @__PURE__ */ new Map();
	for (const w of words) {
		const lw = w.toLowerCase();
		freq.set(lw, (freq.get(lw) ?? 0) + 1);
	}
	const candidates = [...freq.entries()].sort((a, b) => b[1] - a[1] || b[0].length - a[0].length).filter(([w]) => !existing.some((e) => e.word?.toLowerCase() === w)).slice(0, 5).map(([w]) => w);
	const partsOfSpeech = [
		"noun",
		"verb",
		"adjective",
	...
	return candidates.map((word, i) => {
		const exampleSentence = text.split(/(?<=[.!?])\s+/).find((s) => new RegExp(`\\b${word}\\b`, "i").test(s))?.trim() || `This article discusses ${word}.`;
		return {
...
async function normalizeArticle(article) {
	...
	if (finalVocab.length < 3) finalVocab = generateFallbackVocab([
		summary,
		mainStory,
		decClean(currentStory.background),
		decClean(currentStory.expert_analysis)
	].filter(Boolean).join(" "), article.title || "", finalVocab);
	return {
		...article,
...
var listComments = createServerFn({ method: "GET" }).inputValidator((d) => objectType({ articleId: stringType().uuid() }).parse(d)).handler(listComments_createServerFn_handler, async ({ data }) => {
	const supabase = publicClient();
	const { data: rows, error } = await supabase.from("comments").select("id,article_id,user_id,parent_id,prompt_type,body,like_count,created_at,profiles!comments_user_id_fkey(username,display_name,avatar_url)").eq("article_id", data.articleId).eq("is_hidden", false).order("created_at", { ascending: true }).limit(200);
	if (error) {
		const { data: alt } = await supabase.from("comments").select("id,article_id,user_id,parent_id,prompt_type,body,like_count,created_at").eq("article_id", data.articleId).eq("is_hidden", false).order("created_at", { ascending: true }).limit(200);
		return alt ?? [];
	}
	return (rows ?? []).map((r) => ({
		...r,
...
//#endregion
export { getArticleBySlug_createServerFn_handler, getBriefingToday_createServerFn_handler, getCountryStats_createServerFn_handler, getFeatured_createServerFn_handler, getRelated_createServerFn_handler, listArticles_createServerFn_handler, listComments_createServerFn_handler, searchArticles_createServerFn_handler };
