...
//#region node_modules/.nitro/vite/services/ssr/assets/articles.functions-B14ONder.js
...
var bumpLike = createServerFn({ method: "POST" }).inputValidator((d) => objectType({ commentId: stringType().uuid() }).parse(d)).handler(bumpLike_createServerFn_handler, async ({ data }) => {
	const { error } = await publicClient().rpc("bump_comment_likes", { comment_id: data.commentId });
	if (error) throw new Error(error.message);
	return { ok: true };
});
