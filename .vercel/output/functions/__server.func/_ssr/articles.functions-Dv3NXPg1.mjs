...
//#region node_modules/.nitro/vite/services/ssr/assets/articles.functions-Dv3NXPg1.js
...
var getRelated = createServerFn({ method: "GET" }).inputValidator((d) => objectType({
	...
	const supabase = publicClient();
	const { data: rows, error } = await supabase.from("articles").select(summaryCols).eq("is_published", true).eq("category", data.category).neq("slug", data.excludeSlug).order("published_at", { ascending: false }).order("id", { ascending: false }).limit(data.limit);
	if (error) throw new Error(error.message);
	let result = rows ?? [];
	if (result.length === 0) {
		const { data: fallback } = await supabase.from("articles").select(summaryCols).eq("is_published", true).neq("slug", data.excludeSlug).order("published_at", { ascending: false }).order("id", { ascending: false }).limit(data.limit);
		result = fallback ?? [];
	}
	return result;
});
