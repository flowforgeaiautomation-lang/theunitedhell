...
async function normalizeArticle(article: Article): Promise<Article> {
  ...
  // Filter out vocabulary entries with placeholder/generic meanings or broken examples.
  const BAD_VOCAB_MEANING = /\b(important word|word used|used in this story|key word|significant term)\b/i;
  ...
  })).filter((v: any) => v.word && v.meaning && !BAD_VOCAB_MEANING.test(v.meaning)) || [];

  return {
    ...article,
    title: cleanStoryText(article.title) ?? dec(article.title) ?? article.title,
    ...
    story: {
      ...
      vocabulary: await buildVocabularyFromText(allText, rawVocab),
