...
import { n as lookupWord, t as getPopularWords } from "./dictionary.server-CJ6qJACk.js";
import { z } from "zod";
//#region src/lib/word-search.functions.ts?tss-serverfn-split
...
var searchWord = createServerFn({ method: "GET" }).inputValidator((d) => z.object({ word: z.string().min(1).max(80) }).parse(d)).handler(searchWord_createServerFn_handler, async ({ data }) => {
...
var popularWords = createServerFn({ method: "GET" }).inputValidator((d) => z.object({ limit: z.number().int().min(1).max(20).default(8) }).parse(d ?? {})).handler(popularWords_createServerFn_handler, async ({ data }) => {
