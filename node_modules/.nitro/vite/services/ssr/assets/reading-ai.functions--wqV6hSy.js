...
import { orChat } from "./openrouter.server-49lyNjab.js";
import { z } from "zod";
//#region src/lib/reading-ai.functions.ts?tss-serverfn-split
/**
...
var aiTranslateSelection = createServerFn({ method: "POST" }).inputValidator((d) => z.object({
	text: z.string().min(1).max(2e3),
	target: z.string().min(2).max(10)
}).parse(d)).handler(aiTranslateSelection_createServerFn_handler, async ({ data }) => {
	const { translateVisibleText } = await import("./translation.functions-CCDV5izf.js").then((n) => n.n);
