...
var generateArticles = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({
...
var curateNow = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({
	maxItems: z.number().int().min(1).max(120).default(36),
	category: z.string().optional()
...
var curateNowPublic = createServerFn({ method: "POST" }).inputValidator((d) => z.object({
	maxItems: z.number().int().min(1).max(20).default(12),
	category: z.string().optional()
...
var reprocessArticles = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({ limit: z.number().int().min(1).max(20).default(8) }).parse(d ?? {})).handler(reprocessArticles_createServerFn_handler, async ({ data }) => {
