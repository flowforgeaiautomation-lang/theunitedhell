...
import { orJson } from "./openrouter.server-DBp1OwsK.js";
import { z } from "zod";
import { createClient } from "@supabase/supabase-js";
//#region src/lib/quiz.functions.ts?tss-serverfn-split
...
var getQuiz = createServerFn({ method: "GET" }).inputValidator((d) => z.object({ articleId: z.string().uuid() }).parse(d)).handler(getQuiz_createServerFn_handler, async ({ data }) => {
...
var unsaveWord = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({ word: z.string().min(1).max(100) }).parse(d)).handler(unsaveWord_createServerFn_handler, async ({ data, context }) => {
	const { supabase, userId } = context;
...
var checkSavedWord = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({ word: z.string().min(1).max(100) }).parse(d)).handler(checkSavedWord_createServerFn_handler, async ({ data, context }) => {
	const { supabase, userId } = context;
...
var toggleCommentLike = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({ commentId: z.string().uuid() }).parse(d)).handler(toggleCommentLike_createServerFn_handler, async ({ data, context }) => {
	const { supabase, userId } = context;
