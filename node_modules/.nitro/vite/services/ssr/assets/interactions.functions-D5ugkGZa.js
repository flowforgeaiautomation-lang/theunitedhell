...
import { z } from "zod";
//#region src/lib/interactions.functions.ts?tss-serverfn-split
...
var toggleLike = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({ articleId: z.string().uuid() }).parse(d)).handler(toggleLike_createServerFn_handler, async ({ data, context }) => {
...
var toggleBookmark = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({ articleId: z.string().uuid() }).parse(d)).handler(toggleBookmark_createServerFn_handler, async ({ data, context }) => {
	const { supabase, userId } = context;
...
var getMyInteractions = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({ articleIds: z.array(z.string().uuid()) }).parse(d)).handler(getMyInteractions_createServerFn_handler, async ({ data, context }) => {
...
var saveInterests = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({ interests: z.array(z.string().min(1)).max(30) }).parse(d)).handler(saveInterests_createServerFn_handler, async ({ data, context }) => {
	const { supabase, userId } = context;
...
var deleteComment = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({ commentId: z.string().uuid() }).parse(d)).handler(deleteComment_createServerFn_handler, async ({ data, context }) => {
	const { supabase, userId } = context;
