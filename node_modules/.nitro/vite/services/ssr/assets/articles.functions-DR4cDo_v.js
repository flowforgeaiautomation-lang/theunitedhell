...
import { o as relatedCategorySlugs } from "./categories-BEROsZZ5.js";
import { r as lookupWords } from "./dictionary.server-CJ6qJACk.js";
import { z } from "zod";
...
var listArticles = createServerFn({ method: "GET" }).inputValidator((d) => z.object({
...
var getArticleBySlug = createServerFn({ method: "GET" }).inputValidator((d) => z.object({ slug: z.string().min(1) }).parse(d)).handler(getArticleBySlug_createServerFn_handler, async ({ data }) => {
	const supabase = publicClient();
...
var getRelated = createServerFn({ method: "GET" }).inputValidator((d) => z.object({
	category: z.string(),
...
var searchArticles = createServerFn({ method: "GET" }).inputValidator((d) => z.object({ q: z.string().min(1).max(120) }).parse(d)).handler(searchArticles_createServerFn_handler, async ({ data }) => {
	const supabase = publicClient();
...
var postReflection = createServerFn({ method: "POST" }).inputValidator((d) => z.object({
...
var bumpLike = createServerFn({ method: "POST" }).inputValidator((d) => z.object({
...
var editComment = createServerFn({ method: "POST" }).inputValidator((d) => z.object({
	commentId: z.string().uuid(),
	body: z.string().trim().min(1).max(4e3)
...
var getLikedComments = createServerFn({ method: "GET" }).inputValidator((d) => z.object({
	articleId: z.string().uuid(),
	userId: z.string().uuid().nullable().optional()
...
var deleteCommentAnon = createServerFn({ method: "POST" }).inputValidator((d) => z.object({ commentId: z.string().uuid() }).parse(d)).handler(deleteCommentAnon_createServerFn_handler, async ({ data }) => {
...
var getArticleImages = createServerFn({ method: "GET" }).inputValidator((d) => z.object({
	slug: z.string().min(1),
	title: z.string(),
	category: z.string()
}).parse(d)).handler(getArticleImages_createServerFn_handler, async ({ data }) => {
	const { pexelsImage } = await import("./openrouter.server-qFv0nSp-.js");
...
var listComments = createServerFn({ method: "GET" }).inputValidator((d) => z.object({
	articleId: z.string().uuid(),
	sort: z.enum([
		"newest",
		"oldest",
