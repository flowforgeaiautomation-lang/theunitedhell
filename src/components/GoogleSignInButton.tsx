import { supabase } from "@/integrations/supabase/client";
import { toast } from "sonner";
import { useState } from "react";

const GoogleIcon = ({ className }: { className?: string }) => (
  <svg className={className} viewBox="0 0 24 24" aria-hidden="true">
    <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
    <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
    <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" />
    <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
  </svg>
);

export function GoogleSignInButton({
  redirectTo,
  label = "Continue with Google",
  className = "",
}: {
  redirectTo?: string;
  label?: string;
  className?: string;
}) {
  const [busy, setBusy] = useState(false);

  async function handleGoogle() {
    setBusy(true);
    try {
      const returnTo = redirectTo || window.location.pathname + window.location.search;
      const { error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo: `${window.location.origin}/auth?redirect=${encodeURIComponent(returnTo)}`,
        },
      });
      if (error) throw error;
    } catch (e) {
      const msg = (e as Error).message;
      if (msg.includes("popup") || msg.includes("closed")) {
        toast.error("Google sign-in was cancelled. Please try again.");
      } else if (msg.includes("provider") || msg.includes("not enabled")) {
        toast.error("Google sign-in is not yet configured. Please use email sign-in for now.");
      } else {
        toast.error("Could not connect to Google. Please try again.");
      }
      setBusy(false);
    }
  }

  return (
    <button
      onClick={handleGoogle}
      disabled={busy}
      className={`w-full flex items-center justify-center gap-3 border-2 border-foreground py-3 text-sm font-semibold uppercase tracking-wider hover:bg-foreground hover:text-background transition disabled:opacity-40 rounded-sm ${className}`}
    >
      {busy ? (
        <span className="inline-block h-5 w-5 border-2 border-current border-t-transparent rounded-full animate-spin" />
      ) : (
        <GoogleIcon className="h-5 w-5" />
      )}
      {busy ? "Connecting…" : label}
    </button>
  );
}
