...
function EpaperPage() {
  ...
  const auth = useAuth();
  const isAuthenticated = auth.loading ? true : auth.isAuthenticated;

  useEffect(() => {
    if (!auth.loading && auth.isAuthenticated && showAuthPrompt) {
      setShowAuthPrompt(false);
    }
  }, [auth.loading, auth.isAuthenticated, showAuthPrompt]);
  ...
  const goToPage = useCallback((idx: number) => {
    ...
    if (idx > 0 && !auth.loading && !auth.isAuthenticated) {
  ...
  }, [currentPage, totalPages, auth.loading, auth.isAuthenticated]);
  ...
  function onTouchEnd(e: React.TouchEvent) {
    ...
    if (Math.abs(dx) > 60) {
      if (dx < 0) { if (currentPage + 1 > 0 && !auth.loading && !auth.isAuthenticated) setShowAuthPrompt(true); else nextPage(); }
      else prevPage();
    }
  }
  ...
  return (
    ...
    >
      ...
      {/* Continue Reading — only on first page for guests */}
      {currentPage === 0 && !auth.loading && !auth.isAuthenticated && (
        <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          >
            <Lock className="h-8 w-8 mx-auto mb-3 text-foreground" />
            <h3 className="font-serif text-xl font-bold mb-1">Continue Reading — Create a Free Account</h3>
            <p className="text-sm text-muted-foreground">You've read the free front page. Create an account or log in to read all {totalPages} pages — completely free.</p>
      ...
      {auth.isAuthenticated && !auth.loading && (
        <div className="max-w-7xl mx-auto px-4 py-2">
          <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-green-600 dark:text-green-400">
            <BookOpen className="h-3.5 w-3.5" /> Full Edition Unlocked
          </span>
        </div>
      )}
            ...
            {pages.map((p, i) => (
              <button
                ...
                className={`h-2 rounded-full transition-all shrink-0 ${
                  i === currentPage ? "bg-foreground w-6" : i > 0 && !auth.loading && !auth.isAuthenticated ? "bg-amber-400/40 hover:bg-amber-400 w-2" : "bg-muted-foreground/30 hover:bg-muted-foreground/60 w-2"
                }`}
                aria-label={`Go to page ${i + 1}: ${p.sectionLabel}${i > 0 && !auth.loading && !auth.isAuthenticated ? " (Sign in required)" : ""}`}
              />
            ))}
          </div>
            ...
            <div className="p-4 space-y-1">
              {pages.map((p, i) => (
                ...
                >
                  ...
                  {!auth.loading && !auth.isAuthenticated && i > 0 && <Lock className="h-3 w-3 text-amber-500 shrink-0" />}
      ...
      {/* Auth Prompt Modal */}
      <AuthPromptModal open={showAuthPrompt} onClose={() => setShowAuthPrompt(false)} redirectTo="/epaper" />
    </div>
  );
}
