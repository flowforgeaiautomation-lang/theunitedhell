...
export const listMyBookmarks = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await supabase
      .from("bookmarks")
      .select("created_at, articles(id,slug,title,dek,category,cover_image_url,read_time_minutes,trust_score,published_at,view_count,like_count,bookmark_count,comment_count,featured_slot,country_code,source_count,subcategory)")
      .eq("user_id", userId)
