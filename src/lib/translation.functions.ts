...
async function doTranslate(texts: string[], source: string, target: string): Promise<string[]> {
  ...
  // 1. LibreTranslate (self-hosted or edge function proxy) — primary when configured
  if (ltUrl) {
    try {
  ...
  // 2. AI translation (best quality, handles all languages) — primary when no LibreTranslate
  try {
    return await aiTranslate(texts, target);
  } catch (e) { errors.push(`AI: ${(e as Error).message}`); }

  // 3. Google Translate free (no API key needed, works for all languages)
  try {
  ...
  // All providers failed — throw so the UI shows the error
