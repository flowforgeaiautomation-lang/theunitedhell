/*
# Update trigger to neutralize source names for future articles

The trigger now:
1. Replaces any source name with "The United Hell" (no external news platform names)
2. Replaces any cover_image_url with a Pexels stock photo if it's not from Pexels
3. Cleans all text fields using clean_article_text
4. Uses extract_difficult_vocabulary
*/
...
BEGIN
  ...
  -- NEUTRALIZE source names: replace ALL source names with "The United Hell"
  ...
  -- REPLACE cover_image_url if not from Pexels
  ...
  -- vocabulary: use extract_difficult_vocabulary
