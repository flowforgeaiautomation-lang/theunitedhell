-- Fix: handle null array_agg in FOREACH for registering words
...
BEGIN
  ...
  -- Register chosen words as used (handle null array_agg)
  SELECT array_agg(elem->>'word') INTO words_to_register
  FROM jsonb_array_elements(result) AS elem;
  IF words_to_register IS NOT NULL THEN
    FOREACH w IN ARRAY words_to_register LOOP
      IF w IS NOT NULL THEN
        INSERT INTO used_vocabulary_words (word) VALUES (w) ON CONFLICT DO NOTHING;
      END IF;
    END LOOP;
