/*
# Backfill vocabulary for articles with partial vocabulary

## Purpose
266 published articles have fewer than 4 vocabulary entries in their story JSON.
This function identifies words from article text, looks them up in the
vocabulary_cache table, and adds missing entries to bring every article up to
at least 4 vocabulary words.

## What it does
1. Finds articles with < 4 vocabulary entries
2. Extracts candidate words (7+ char words) from the article's title, dek, and story text
3. Looks up each word in the vocabulary_cache table
4. Adds up to 5 vocabulary entries to the article's story JSON
5. Processes in batches to avoid timeouts

## Safety
- Only updates articles that have < 4 vocabulary entries
- Never removes existing vocabulary entries
- Only adds words that exist in vocabulary_cache with a meaning
...
BEGIN
  -- Count remaining articles with partial vocabulary
  ...
  LOOP
    ...
    -- Extract candidate words (7+ chars, alphabetic)
    SELECT array_agg(DISTINCT w) INTO candidate_words
    FROM regexp_matches(lower(article_text), '\b[a-z][a-z-]{6,}\b', 'g') AS matches(w)
    WHERE NOT (w = ANY(blocked_words))
    AND char_length(w) >= 7
    LIMIT 30;

    -- Look up each word in vocabulary_cache
    ...
    -- Update the article if we found enough new vocabulary
