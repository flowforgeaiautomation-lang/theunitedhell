/*
# Fix backfill_article_vocabulary function

Fixes the type error in the word filtering query — regexp_matches returns text,
not text[], so the comparison needs to be done differently.
*/
...
BEGIN
  ...
  LOOP
    ...
    -- Extract candidate words using a simpler approach
    candidate_words := ARRAY(
      SELECT DISTINCT w
      FROM regexp_matches(lower(article_text), '\b[a-z][a-z-]{6,}\b', 'g') AS t(w)
      WHERE char_length(w) >= 7
      AND position(' ' || w || ' ' in ' ' || blocked_text || ' ') = 0
      LIMIT 30
    );
