/*
# Fix organizations field (string -> JSON array) and upgrade trigger

1. Problem
   - The organizations field in story is stored as a string (JSON text) instead of a proper JSON array
   - This causes the frontend to fail to render the organizations section
   - The trigger function was storing it as a string due to a bug in the SELECT INTO logic

2. Fix
   - Update the trigger to always store organizations as a proper JSONB array
   - Fix all 451 existing articles to convert organizations from string to JSON array
   - Also fix the sources field inside story to be a proper array of objects

3. Quality upgrades
   - Improve vocabulary extraction to use article-specific words
   - Improve people extraction with better filtering
   - Improve key_numbers extraction to find real numbers in the text
*/
...
BEGIN
  ...
  IF org IS NULL OR jsonb_typeof(org) != 'array' THEN
    -- It's stored as a string or missing, rebuild from sources
  ...
  -- Fix sources inside story: ensure it's a proper JSONB array of objects
