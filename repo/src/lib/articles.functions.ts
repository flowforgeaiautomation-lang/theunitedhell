...
const summaryCols =
  "id,slug,title,dek,category,subcategory,cover_image_url,read_time_minutes,country_code,featured_slot,published_at,view_count,like_count,bookmark_count,comment_count";
...
function cleanStoryText(text?: string | null) {
  ...
  const cleaned = decoded
    ...
    .replace(/\b(Reuters|BBC(?:\s+News)?|GNews|NewsAPI|The Hindu|Times of India|Associated Press|AP News|The Associated Press|The Guardian|New York Times|NYT|CNN|Al Jazeera|Bloomberg|Financial Times|Washington Post|NPR|Fox News|Sky News|France ?24|Deutsche Welle|DW|NDTV|Hindustan Times|Indian Express)\b/gi, "")
    .replace(/[ \t]{2,}/g, " ")
    .trim();
...
async function buildVocabularyFromText(text: string, existing?: Article["story"]["vocabulary"]): Promise<VocabEntry[]> {
  const normalized = normalizeText(text);
  const blocked = new Set(["article", "source", "report", "update", "people", "official", "information", "development", "government", "national", "regional", "general", "several", "however", "various", "whether", "against", "already", "although", "instead", "despite", "further", "certain", "because", "through", "between", "another", "current", "reported", "statement", "including", "thursday", "friday", "wednesday", "tuesday", "monday", "saturday", "sunday", "january", "february", "march", "april", "may", "june", "july", "august", "september", "october", "november", "december"]);
  const keep = (existing || [])
    .filter((v) => v?.word && v.meaning && normalized.includes(normalizeText(v.word)))
    .filter((v) => !/an important (word|term) used in this story/i.test(v.meaning ?? ""))
  ...
  const candidates = Array.from(new Set((text.match(/\b[A-Za-z][A-Za-z-]{6,}\b/g) || []).map((w) => w.replace(/^-|-$/g, ""))))
    .filter((word) => !blocked.has(word.toLowerCase()))
    .filter((word) => !keep.some((v) => normalizeText(v.word) === normalizeText(word)))
    .slice(0, 20);
...
async function normalizeArticle(article: Article): Promise<Article> {
  ...
  const summary = decClean((currentStory as any).summary) || decClean(article.dek);
  const proseParts = [
  ...
  const allText = [summary, mainStory, decClean((currentStory as any).background), decClean((currentStory as any).expert_analysis)].filter(Boolean).join(" ");
  return {
    ...article,
    ...
    story: {
      ...
      key_developments: decodeListMaybe(keyDevelopments),
      quick_insights: decodeListMaybe(quickInsights),
      ...
      vocabulary: await buildVocabularyFromText(allText, (currentStory as any).vocabulary?.map((v: any) => ({
        word: dec(v.word) || undefined,
        partOfSpeech: v.part_of_speech || v.partOfSpeech || undefined,
        meaning: dec(v.meaning) || undefined,
        simpleExplanation: dec(v.simple_explanation) || dec(v.simpleExplanation) || undefined,
