...
import { Search, X, Volume2, History, TrendingUp, ChevronDown } from "lucide-react";
import { searchWord, popularWords } from "@/lib/word-search.functions";
import type { VocabEntry } from "@/lib/types";

const RECENT_KEY = "tuh_word_search_recent";
...
function saveRecent(words: string[]) {
  if (typeof window === "undefined") return;
  try {
    localStorage.setItem(RECENT_KEY, JSON.stringify(words.slice(0, RECENT_LIMIT)));
  } catch {
    /* ignore */
  }
}
...
export function WordSearch() {
  ...
  const { data, isLoading, isFetching } = useQuery({
    queryKey: ["word-search", debounced],
    queryFn: () => runSearch({ data: { word: debounced } }),
  ...
  const commitRecent = (word: string) => {
    const w = word.trim();
    if (!w) return;
    const next = [w, ...recent.filter((r) => r.toLowerCase() !== w.toLowerCase())].slice(0, RECENT_LIMIT);
    setRecent(next);
    saveRecent(next);
  };
  ...
  const showResult = debounced.length >= 2 && (isLoading || isFetching || data !== undefined);
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        <label htmlFor="tuh-word-search" className="sr-only">
          Search any word
        </label>
        <Search
        ...
        <input
          ...
          onBlur={() => query.trim() && commitRecent(query.trim())}
    ...
    <div className="overflow-hidden rounded-xl border rule bg-background shadow-sm">
      ...
      >
        <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
          ...
          {entry.pronunciation && (
            <span className="text-sm italic text-muted-foreground">{entry.pronunciation}</span>
          )}
          {entry.partOfSpeech && (
            <span className="border rule px-2 py-0.5 text-[0.65rem] uppercase tracking-widest text-muted-foreground">
              {entry.partOfSpeech}
            </span>
          )}
        </div>
      ...
      {open && (
        <div className="space-y-3 border-t rule p-5">
          ...
          {entry.simpleExplanation && (
            <p className="text-sm leading-relaxed text-foreground/70">{entry.simpleExplanation}</p>
          )}
          {entry.example && (
            <p className="border-l-2 border-foreground/10 pl-3 text-sm italic leading-relaxed text-muted-foreground">
              {entry.example}
            </p>
          )}
